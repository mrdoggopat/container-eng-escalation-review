...
const TOKENIZER = /(".*?"|>=|<=|!=|=|\bAND\b|\bOR\b|\bNOT\b|\bIN\b|\bIS\b|\bEMPTY\b|\bNULL\b|\bDURING\b|\bDone\b|\bstatusCategory\b|\bstatus\b|\bchanged\b|\bto\b|\bproject\b|\bcreated\b)/g;
    ...
    <div className="prepare-page">
      ...
      <section className="query-list">
        <QueryBlock
          ...
          description={
            `Cards whose status history transitions into ${jqlConfig.escalationColumns.map((c) => `"${c}"`).join(", ")} during the window` +
            (jqlConfig.resolvedStatuses && jqlConfig.resolvedStatuses.length > 0
              ? ` AND whose current status is ${jqlConfig.resolvedStatuses.map((s) => `"${s}"`).join(" or ")} (in-flight cards are excluded).`
              : ` AND whose current statusCategory is "Done" (resolved cards only — in-flight escalations are excluded).`)
          }
          jql={escalationJql}
        />
        <QueryBlock
